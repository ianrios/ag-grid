...
export class PivotStage extends BeanStub implements IRowNodeStage {
    ...
    private executePivotOn(changedPath: ChangedPath): void {
        ...
        this.maxUniqueValues = this.gridOptionsService.get('pivotMaximumGeneratedColumns') / numberOfAggregationColumns;
        let uniqueValues;
        try {
            ...
            if (e.message === PivotStage.EXCEEDED_MAX_UNIQUE_VALUES) {
                ...
                const event: WithoutGridCommon<PivotLimitExceededEvent> = {
                    type: Events.EVENT_PIVOT_LIMIT_EXCEEDED,
                    message: e.message,
                };
