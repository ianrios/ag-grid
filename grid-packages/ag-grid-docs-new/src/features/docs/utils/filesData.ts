...
export const getPageImages = async ({
    ...
    const images = import.meta.glob<{ default: ImageMetadata }>('../../../content/docs/**/*.{jpeg,jpg,png,gif}');

    if (!images[fullImagePath]) {
        const errorMsg = `Page "${pageName}" image "${imagePath}" does not exist in glob: "${docsPath}**/*.{jpeg,jpg,png,gif}" (fullImagePath = ${fullImagePath})`;
        if (getIsDev()) {
