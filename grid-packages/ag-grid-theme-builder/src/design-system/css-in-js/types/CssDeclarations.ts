...
type ColorProperty =
  | 'color'
  // extract keys of StandardProperties that contain the substring 'Color'
  | {
      [K in keyof StandardProperties]: K extends `${infer _prefix}Color${infer _suffix}`
        ? K
        : never;
    }[keyof StandardProperties];

type DirectionalProperty =
  | 'top'
  | 'left'
  | 'right'
  | 'bottom'
  // extract keys of StandardProperties that contain the substring 'Color'
  | {
      [K in keyof StandardProperties]: K extends `${infer _prefix}${
        | 'Top'
        | 'Bottom'
        | 'Left'
        | 'Right'
        | 'Start'
        | 'End'
        | 'Inline'}${infer _suffix}`
        ? K
        : never;
    }[keyof StandardProperties];

// extract keys of StandardProperties that are lengths or time quantities (this works because
// StandardProperties is a generic type that takes arguments specifying the type of lengths
// and durations)
type DimensionProperty = {
  [K in keyof StandardProperties]-?: { _: 0 } extends StandardProperties<{ _: 0 }, { _: 0 }>[K]
    ? K
    : never;
}[keyof StandardProperties];
